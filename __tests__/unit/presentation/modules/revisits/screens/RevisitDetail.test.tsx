...
describe('Test in <RevisitDetail /> screen', () => {
    ...
    it('should render revisit Time', () => {
        ...
        /* Check if Time exists and contain respective value */
        expect(createdDate).toBeOnTheScreen();
        expect(createdDate).toHaveTextContent(Time.format(selectedRevisit.createdAt, 'DD/MM/YYYY'));
    });

    it('should render next visit when selectedRevisit.done is false', () => {
        renderScreen();
        const nextVisit = Time.format(selectedRevisitStateMock.selectedRevisit.nextVisit, 'DD [de] MMMM [del] YYYY');
